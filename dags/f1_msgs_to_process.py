...
def download_and_process_files(**kwargs):
    try:
        ...
        bigquery_files = kwargs["ti"].xcom_pull(
            task_ids="t2_check_msg_files_in_process"
        )
        if not gsc_files:
...
with DAG(
    ...
    update_status_table = PythonOperator(
        ...
        op_kwargs={
            "file_path": "{{ ti.xcom_pull(task_ids='t3_get_files_to_process_task') }}"
        },
    )
