...
logging.info('Initializing TelegramParser and StorageManager')
...
def parse_tg_dialogs():
    try:
        logging.info('Start parsing tg dialogs')
        json_helper.save_to_json(data=tg_parser.get_channels(), path=TG_CHANNELS_LOCAL_PATH)
...
def update_target_ids(date=vars.START_DATE, force=False):
    try:
...
def update_metadata_in_cloud():
    try:
        ...
        logging.info(f'Updated cloud metadata with {MG_CHANNELS_LOCAL_PATH}. Now you can use it')
        return True
    except Exception as e:
...
with DAG(
    ...
    update_last_updated_ids_task = PythonOperator(
        task_id='t4_update_last_updated_ids',
        python_callable=update_last_updated_ids
    )
